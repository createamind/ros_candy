...
class MultiModal(Module):
    ...
    def _build_net(self, is_training, reuse):
        ...
        # self.parts = []
        #Encoder
        self.e1 = ImageEncoder(self.camera_left, self._args, 'encodercl', is_training=is_training, reuse=reuse).outputs
        self.e2 = ImageEncoder(self.camera_right, self._args, 'encodercr', is_training=is_training, reuse=reuse).outputs
        self.e3 = ImageEncoder(self.eye_left, self._args, 'encoderel', is_training=is_training, reuse=reuse).outputs
        self.e4 = ImageEncoder(self.eye_right, self._args, 'encoderer', is_training=is_training, reuse=reuse).outputs
        ...
        z_size = self.sample_z.shape.as_list()[1] // 4
        self.vae_loss = VAELoss(self.mean[:,:z_size], self.logsigma[:,:z_size], self._args, 'vae_loss', is_training=is_training, reuse=reuse)
        ...
        beta = tf.get_variable('beta', shape=(), initializer=tf.constant_initializer([5]), trainable=False, dtype=tf.float32)
        new_beta = tf.assign(beta, tf.minimum(1.001 * beta, 5))
