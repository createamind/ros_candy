...
class MultiModal(Module):
    ...
    def _build_net(self, is_training, reuse):
        ...
        self.e1 = tf.layers.dropout(self.encoder_cl.outputs, rate=0.00001, training=is_training)
        self.e2 = tf.layers.dropout(self.encoder_cr.outputs, rate=0.00001, training=is_training)
        self.e3 = tf.layers.dropout(self.encoder_el.outputs, rate=0.00001, training=is_training)
        self.e4 = tf.layers.dropout(self.encoder_er.outputs, rate=0.00001, training=is_training)
        ...
        self.z = tf.concat([self.mean1, self.mean2, self.mean3, self.mean4], 1)
